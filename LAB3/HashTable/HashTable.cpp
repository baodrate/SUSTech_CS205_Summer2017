...
{
    int index = hash( newItem -> size );
    array[ index ].insertItem( newItem );
}

// Deletes an Item by key from the Hash Table.
// Returns true if the operation is successful.
bool HashTable::removeItem( Item * item )
{
    int index = hash( item -> size );
    return array[ index ].removeItem( item -> key );
}

// Returns an item from the Hash Table by key.
// If the item isn't found, a null pointer is returned.
Item * HashTable::getItemByKeyandSize( string itemKey,int itemSize )
{
    int index = hash( itemSize );
